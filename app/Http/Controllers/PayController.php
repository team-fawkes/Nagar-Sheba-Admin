...
class PayController extends Controller
{
    public function payment($invid){
        ...
        $tran_id = $invid;//unique transection id for every transection
    ...
    public function success(Request $request){
        ...
        $response = curl_exec($curl);
        curl_close($curl);
        $invoice  =  Invoice::where('invid',$request->mer_txnid)->first();
        $invoice->status = 'success';
        $invoice->trxid = $request->pg_txnid;
        $invoice->update();

        return 'Payment is complete';
//        echo $response->mer_txnid;
